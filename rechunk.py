...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--r_prototype", type=str, default="", help="glob-parsable prototype of files to read."
    )
    parser.add_argument(
        "-s", "--sky_cmp", type=str, default="", help="Sky component (e.g. diffuse)."
    )
    parser.add_argument(
        "--n_times_to_load", type=int, default=2160, help="Number of integrations to load."
    )
    parser.add_argument(
        "--n_stride", type=int, default=0, help="Number of n_times_to_load integrations to skip."
    )
    parser.add_argument(
        "--chunk_size", type=int, default=180, help="Number of integrations per chunk."
    )
    parser.add_argument(
        "--array_layout", type=str, default="", help="Path to full array layout."
    )
    parser.add_argument(
        "--lst_wrap", type=float, default=np.pi, help="Where to perform the wrap in LST."
    )
    parser.add_argument(
        "--clobber", default=False, action="store_true", help="Overwrite existing files."
    )
    parser.add_argument(
        "--compress", default=False, action="store_true", help="Compress by redundancy."
    )
    parser.add_argument(
        "--inflate", default=False, action="store_true", help="Inflate by redundancy."
    )
    parser.add_argument('--loglevel', choices=['ERROR', 'WARNING', 'INFO', 'DEBUG'],
                        default='INFO')    
    ...
    if args.inflate and args.compress:
        raise RuntimeError("Cannot both inflate and compress.")
    ...
    if args.r_prototype:
        ...
        raw_files = sorted(list(base_dir.iterdir()))
