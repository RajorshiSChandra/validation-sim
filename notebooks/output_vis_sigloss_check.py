...
if cross_outfile.exists() and autos_outfile.exists():
    ...
    print("\n[WARN] One or both concatenated files do not exist.")
    if not cross_outfile.exists():
        print("  Missing:", cross_outfile)
    if not autos_outfile.exists():
        print("  Missing:", autos_outfile)
    print("  -> Run the concatenation cell first to create them.")
...
spw_ranges = [(fch_min-fch_min, fch_max-fch_min)]
...
uvp = ds.pspec(baselines, baselines, (0, 0),
               [polpair_in],
               spw_ranges=spw_ranges, input_data_weight='identity',
               norm='I',
               taper= 'blackman-harris',
               verbose=True)
...
def replace_nan_with_zero_uvp(uvp):
    """
...
uvp_clean, before, after = replace_nan_with_zero_uvp(uvpinc)
for name in before:
...
uvpspec_averaged = average_spectra(
    ...
    error_field = 'autos_diag',
    time_avg=False,              # Set to True if you also want to average across time
    inplace=False                # Returns a new averaged UVPSpec object
)
...
output_file = f"notebook_{sky_type}_{ideal_tag}_{airy_tag}_Coh_PSPEC_{bl_len}_{bl_ang}_{pol}_ck{chunk_min:05d}-{chunk_max:05d}_{proc1[0]}_{proc2[0]}_bch_{batchnum}_psc_PN.h5"
...
output_file = f"notebook_{sky_type}_{ideal_tag}_{airy_tag}_Incoh_PSPEC_{bl_len}_{bl_ang}_{pol}_ck{chunk_min:05d}-{chunk_max:05d}_{proc1[0]}_{proc2[0]}_bch_{batchnum}_psc_PN.h5"
